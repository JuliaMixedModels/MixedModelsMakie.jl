...
struct ShrinkageInfo{T<:AbstractFloat}
    ...
    """
    blimps::Matrix{T}
    """The (lower-triangular) relative covariance factor of the random effects for this
       grouping factor, such that the covariance matrix is proportional to λλ'. Shared
       across all levels. Used for constructing correlation ellipses in shrinkage plots.
    """
    λ::Matrix{T}
end
...
function shrinkageinfo(m::MixedModel{T}, gf::Symbol,
                       θref::Vector{T}=_ref_theta(m)) where {T}
...
"""
    ...
    shrinkageplot!(f::$(Indexable), si::ShrinkageInfo;
...
function shrinkageplot!(f::Indexable,
                        ...
                        kwargs...) where {T}
    si = shrinkageinfo(m, gf, θref)
    return shrinkageplot!(f, si; kwargs...)
end
...
function shrinkageplot!(f::Indexable,
    ...
    cols = something(cols, axes(si.cnames, 1))
    cols = _cols_to_idx(si.cnames, cols)

    if length(cols) == 1
        colname = si.cnames[only(cols)]
        msg = if user_specified_single
            "You only specified a single column."
        else
            "Random effect grouping only has a single " *
            "predictor associated with it (\"$(colname)\")."
        end
        msg *= " You need at least two."
        throw(ArgumentError(msg))
    end
    reest = view(si.blups, :, cols)
